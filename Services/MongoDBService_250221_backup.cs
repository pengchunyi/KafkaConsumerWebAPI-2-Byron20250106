...
namespace KafkaConsumerWebAPI.Services
{
	public class MongoDBService
	{
		...
		// [新增2]：一次取 Power/Preheat/Trough 三筆最新，(這是給實時錫爐數據顯示使用的)
		...
		// ✅ 確保這個方法存在 (解決 `QueryCollectionAsync` 未找到的錯誤)
		public async Task<List<BsonDocument>> QueryCollectionAsync(string collectionName, FilterDefinition<BsonDocument> filter)
		{
			var collection = _database.GetCollection<BsonDocument>(collectionName);
